...
export default function SubcategorySlider() {
    ...
    return (
        <div className="container">
            <h3 className='text-center p-2 bg-primary text-light mt-2'>Our Subcategory</h3>
                    ...
                    SubcategoryStateData.map((item) => {
                        return <SwiperSlide key={item.id}>
                            <div className="wow fadeIn m-1" data-wow-delay="0.2s">
                                <div>
                                    <Link className='fs-3 position-absolute bottom-0 text-dark d-block w-100 text-center m-1' to={`/shop?sc=${item.name}`}>{item.name}</Link>
